...
export default function ReviewPage() {
  ...
  const filteredPublications = publications.filter(pub => {
    ...
    const matchesSearch = searchQuery === '' || 
      ...
      pub.excerpt[language].toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div className={rtlClass}>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8 border-l-4 border-[#8FD694]">
            <h2 className="text-3xl font-bold mb-4 text-[#171717]">
              {language === 'fr' ? 'Notre première revue arrive en juillet 2025 !' : 'تصدر مجلتنا الأولى في يوليو 2025!'}
            </h2>
            <p className="text-lg text-gray-700 mb-6">
              {language === 'fr' 
                ? 'Nous avons le plaisir de vous annoncer que la première édition de notre revue sera publiée en juillet 2025. Cette revue trimestrielle abordera les questions juridiques, les droits humains et les enjeux sociaux actuels.' 
            ...
            <h3 className="text-xl font-semibold mb-3 text-[#8FD694]">
              {language === 'fr' ? 'Vous souhaitez contribuer ?' : 'هل ترغب في المساهمة؟'}
            </h3>
            <p className="text-gray-700 mb-6">
              {language === 'fr' 
                ? 'Nous invitons les chercheurs, juristes, académiciens et experts à contribuer à notre revue. Si vous souhaitez soumettre un article ou partager votre expertise, n\'hésitez pas à nous contacter via notre formulaire de contact ou sur nos réseaux sociaux.' 
                : 'ندعو الباحثين والقانونيين والأكاديميين والخبراء للمساهمة في مجلتنا. إذا كنت ترغب في تقديم مقال أو مشاركة خبرتك، فلا تتردد في الاتصال بنا من خلال نموذج الاتصال الخاص بنا أو على وسائل التواصل الاجتماعي.'}
            </p>
        ...
        <div className="container mx-auto px-4">
          <div className={`mb-12 ${textAlign}`}>
            <h2 className="text-4xl font-bold text-[#171717] mb-6">
              {language === 'fr' ? 'Publications récentes' : 'المنشورات الحديثة'}
            </h2>
            
            <div className="flex flex-col md:flex-row md:justify-between md:items-end">
              <p className="text-lg text-gray-700 max-w-2xl mb-6 md:mb-0">
                {language === 'fr' 
                  ? "Découvrez l'ensemble de nos ressources documentaires sur les droits humains et les questions juridiques." 
          ...
          <div className="mb-12 flex flex-wrap gap-3 justify-center">
            {publicationCategories.map((category) => (
              <button
                key={category.id}
                onClick={() => setActiveCategoryType(category.id)}
                className={`px-6 py-3 rounded-full transition-all ${
                  activeCategoryType === category.id
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className={`mb-12 ${textAlign}`}>
            <h2 className="text-4xl font-bold text-[#171717] mb-6">
              {language === 'fr' ? 'Médiathèque' : 'مكتبة الوسائط'}
            </h2>
            
            <div className="flex flex-col md:flex-row md:justify-between md:items-end">
              <p className="text-lg text-gray-700 max-w-2xl mb-6 md:mb-0">
                {language === 'fr' 
                  ? "Explorez notre collection de ressources audiovisuelles sur les droits humains." 
                  : "استكشف مجموعتنا من الموارد السمعية والبصرية حول حقوق الإنسان."}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className={`lg:col-span-2 ${textAlign}`}>
              <h2 className="text-3xl font-bold text-[#171717] mb-4">
                {language === 'fr' ? 'Publication à la une' : 'منشور مميز'}
            </h2>
              <p className="text-gray-600 mb-6">
              {language === 'fr' 
                  ? "Notre rapport annuel présente un aperçu complet de l'état des droits humains en Algérie." 
